...
本セクションでは，各種敵対生成手法によって生成された攻撃データに対して異なるモデル間での転移性（Transferability）の有無と程度を定量的に評価する．
### 評価指標
...
| ----------------------- | ---------- | --------------------- | ------------------------ | ----------------- |
| FGSM                    | CNN_small | 73.60                 | 61.53                    | 12.07       |
| FGSM                    | CNN        | 81.34                 | 47.95                    | 33.39       |
| FGSM                    | ResNet18   | 88.93                 | 73.97                    | 14.96       |
| UAP                     | CNN_small | 72.25                 | 56.96                    | 15.29       |
| UAP                     | CNN        | 79.94                 | 63.42                    | 16.52       |
| UAP                     | ResNet18   | 88.09                 | 61.83                    | 26.26       |
| Local UAP (withEntropy) | CNN_small | 72.25                 | 37.38                    | 34.87       |
...
表から，FGSMは`CNN_small`で訓練した敵対事例はCNNに対して極めて高い転移性を示した一方で，ResNetに対しては限定的なものとなった．特筆すべきは，`CNN`における精度低下（33.39%）が，攻撃元である`CNN_small`自身の精度低下（12.07%）を上回った点にある．これについての詳細な考察は後ほど行う．    
...
特に転移可能性についての考察は，既存の文献と重複する箇所もあるが，**筆者の個人的な解釈**に基づくものになるため，十分留意されたい．
